...
def game(genome, config):
    ...
    while keyboard.is_pressed('q') == False and running:
        ...
        if((r == 23 and g == 166 and b == 76)or(r == 22 and g == 159 and b == 73)):
            highestScore = (pipeCounter,highestScore)[highestScore>pipeCounter]
            pipeCounter = 0
            if tryCounter > 0:
                print('GAME OVER! Attempt %s Score: %s Highest: %s'%(tryCounter,pipeCounter,highestScore))
            if tryCounter < 5:
        ...
        if (running):
            ...
            fitness = SCORE + (time.time()-start) + (pipeCounter*100)
