...
pipeBottomImage = cv.imread('assets/PipeBottom2.png')
with mss.mss() as sct:
    while keyboard.is_pressed('q') == False and running:
        ...
        floorX, floorY = pyautogui.position()
        floorY = floorY - reg3
        ...
        if ((birdY > floorY or (vel>0 and (birdY + vel + (vel-lastVel) > floorY))) and (afterSpaceCount > 2 or (birdY>floorY+100 and floorX > 55))):
            birdFoundCount=birdFoundCount+1
            iml[birdFoundCount] = ImageGrab.grab(bbox=(915-50+birdX,400-50+birdY-birdYOffset,915-50+birdX+100,400-50+birdY-birdYOffset+100))
            afterSpaceCount = 1
            pressSpace()
        else: 
    ...
    for index, image in enumerate(iml):
        if image:
            image.save(r"C:\Users\wdona\OneDrive\Documents\Code\Python\FlappyBot\ss3\Investigate-%s.png"%(index))